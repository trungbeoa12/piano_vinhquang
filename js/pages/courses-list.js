document.addEventListener('DOMContentLoaded', function () {
  var g = document.getElementById('courses-grid');
  if (!g || !window.PVQ_Content) return;

  g.innerHTML = '<p class="pvq-muted">Đang tải danh sách khóa học...</p>';

  window.PVQ_Content.loadCourses()
    .then(function (courses) {
      g.innerHTML = courses
        .map(function (c) {
          return (
            '<a class="pvq-catalog-card" href="course-detail.html?id=' +
            encodeURIComponent(c.id) +
            '">' +
            '<img class="pvq-catalog-card-image" src="' +
            c.heroImage +
            '" alt="' +
            c.title.replace(/"/g, '') +
            '">' +
            '<div class="pvq-catalog-card-body">' +
            '<h2>' +
            c.title +
            '</h2>' +
            '<div class="pvq-catalog-meta">' +
            c.level +
            ' • ' +
            c.durationWeeks +
            ' tuần • ' +
            c.priceLabel +
            '</div>' +
            '<p>' +
            c.summary +
            '</p>' +
            '<span class="pvq-link-arrow">Xem chương trình →</span>' +
            '</div></a>'
          );
        })
        .join('');
    })
    .catch(function (error) {
      g.innerHTML =
        '<div class="pvq-alert"><p>Không tải được danh sách khóa học.</p></div>';
      console.error('[courses-list] load failed:', error);
    });
});
