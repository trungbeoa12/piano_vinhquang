...
(function () {
  ...
  var KEYBOARD_INSTANCES = [];
  var ACTIVE_CODES = new Set();
  ...
  PianoStudioPlayer.prototype._setupKeyboardPlay = function () {
    ...
    document.addEventListener('keydown', function (e) {
      ...
      var player = KEYBOARD_INSTANCES[0];
      var note;
      ...
      note = keyMap[e.code];
      if (note && !ACTIVE_CODES.has(e.code)) {
        e.preventDefault();
        ACTIVE_CODES.add(e.code);
        player.audio.playNote(note, 0.4);
        player.highlightKey(note);
      }
    });

    document.addEventListener('keyup', function (e) {
      ACTIVE_CODES.delete(e.code);
    });
  };
